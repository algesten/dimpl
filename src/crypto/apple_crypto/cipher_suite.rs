...
use crate::crypto::dtls_aead::{Aad, Nonce};
...
type CCPadding = u32;
#[allow(non_camel_case_types)]
type CCCryptorRef = *mut std::ffi::c_void;
...
#[allow(non_upper_case_globals)]
const kCCModeGCM: CCMode = 11;
#[allow(non_upper_case_globals)]
const ccNoPadding: CCPadding = 0;
...
extern "C" {
    ...
    fn CCCryptorRelease(cryptorRef: CCCryptorRef) -> CCCryptorStatus;

    fn CCCryptorGCMAddIV(
        cryptorRef: CCCryptorRef,
        iv: *const std::ffi::c_void,
        ivLen: usize,
    ) -> CCCryptorStatus;

    fn CCCryptorGCMAddAAD(
        cryptorRef: CCCryptorRef,
        aad: *const std::ffi::c_void,
        aadLen: usize,
    ) -> CCCryptorStatus;
...
impl Cipher for AesGcm {
    fn encrypt(&mut self, plaintext: &mut Buf, aad: Aad, nonce: Nonce) -> Result<(), String> {
        ...
        struct CryptorGuard(CCCryptorRef);
