...
pub trait Cipher {
    ...
    fn encrypt(&self, plaintext: &[u8], aad: &[u8], nonce: &[u8]) -> Result<Vec<u8>, &'static str>;

    /// Decrypt data with additional authenticated data (AAD)
    fn decrypt(&self, ciphertext: &[u8], aad: &[u8], nonce: &[u8])
        ...
        match key.len() {
            16 => {
                let cipher = Aes128Gcm::new_from_slice(key)
                    .map_err(|_| "Failed to create AES-128-GCM cipher")?;
            ...
            _ => Err("Invalid key size for AES-GCM"),
        }
    }
}
...
impl Cipher for AesGcm {
    fn encrypt(&self, plaintext: &[u8], aad: &[u8], nonce: &[u8]) -> Result<Vec<u8>, &'static str> {
        if nonce.len() != 12 {
            return Err("AES-GCM nonce must be 12 bytes");
        }
