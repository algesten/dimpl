// RFC 7919 ffdhe2048 parameters

use crate::crypto::DhDomainParams;

const P: &[u8] = &[
    ,
];

const G: &[u8] = &[0x02];

struct Ffdhe2048Params {
    p: &'static [u8],
    g: &'static [u8],
}

impl DhDomainParams for Ffdhe2048Params {
    fn p(&self) -> &[u8] {
        self.p
    }
    fn g(&self) -> &[u8] {
        self.g
    }
    fn into_p_g(self) -> (Vec<u8>, Vec<u8>) {
        (self.p.to_vec(), self.g.to_vec())
    }
}

pub fn params() -> impl DhDomainParams {
    Ffdhe2048Params { p: P, g: G }
}
