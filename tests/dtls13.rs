...
fn dtls13_selective_retransmit_on_handshake_ack() {
    ...
    let mut client = Dtls::new(Arc::clone(&config), client_cert);
    client.set_active(true);
    let mut server = Dtls::new(config, server_cert);
...
fn dtls13_large_application_data_fragmented() {
    ...
    // With MTU of 300, 1000 bytes should require multiple packets
    // (accounting for headers and encryption overhead)
    assert!(
        packet_count >= 2,
...
fn dtls13_srtp_keying_material_correct_size() {
    ...
    let expected_size = match profile {
        SrtpProfile::AeadAes128Gcm => 2 * (16 + 12), // 2 * (key + salt) for AES-128-GCM
        SrtpProfile::AeadAes256Gcm => 2 * (32 + 12), // 2 * (key + salt) for AES-256-GCM
        SrtpProfile::Aes128CmSha1_80 => 2 * (16 + 14), // 2 * (key + salt) for AES-128-CM
    };
...
fn dtls13_caches_app_data_when_handshake_packet_lost() {
    ...
    for round in 0..100 {
        ...
        if !server_first_flight_sent && !server_out.packets.is_empty() && round > 0 {
            server_first_flight_sent = true;
            let num_packets = server_out.packets.len();
            eprintln!(
                "Round {}: Server sending {} packets in first flight",
                round, num_packets
            );
            ...
            if num_packets > 2 {
                ...
                // Not enough packets to drop one, deliver all
                deliver_packets(&server_out.packets, &mut client);
            }
        } else {
