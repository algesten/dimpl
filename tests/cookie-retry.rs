...
//! processes the retried ClientHello with cookie (which has the same message_seq=0)
//! instead of treating it as a duplicate and resending HelloVerifyRequest.
//!
//! Bug: The server was incrementing peer_handshake_seq_no after the first ClientHello,
//! causing the second ClientHello (with cookie, same msg_seq=0) to be treated as a
//! duplicate, triggering a resend of HelloVerifyRequest instead of cookie validation.
...
fn parse_record_content_types(datagram: &[u8]) -> Vec<u8> {
    let mut out = Vec::new();
    let mut i = 0usize;
...
fn cookie_retry_proceeds_to_server_hello() {
    ...
    // FLIGHT 3: Client sends ClientHello WITH cookie (same message_seq=0)
